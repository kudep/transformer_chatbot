...
class TransformerModule(nn.Module):
    ...
    # def index_search(orign_tensor, spec_id, with_first_items=False, with_last_items=False):
    #     assert not(with_first_items and with_last_items)
    #     tensor = orign_tensor
    #     if with_first_items:
    #         tensor = tensor.clone()
    #         # orign_tensor[:, -1]
    #         # tensor[:, 0] = spec_id  # Add first and last items
    #         tensor[:, 0] = torch.where(tensor[:, -1] == spec_id,  # check opposite component
    #                                    tensor[:, 0],  # if
    #                                    torch.full_like(tensor[:, 0], spec_id, dtype=torch.int64)  # else
    #                                    )  # Exclusive adding
    #     if with_last_items:
    #         tensor = tensor.clone()
    #         # tensor[:, -1] = spec_id  # Add first and last items
    #         tensor[:, -1] = torch.where(tensor[:, 0] == spec_id,  # check opposite component
    #                                     tensor[:, -1],  # if
    #                                     torch.full_like(tensor[:, -1], spec_id, dtype=torch.int64)  # else
    #                                     )  # Exclusive adding
    #     found_indexes = (tensor == spec_id).nonzero()
    #     return found_indexes  # return [[dim1_index, dim2_index]]

    # def segment_search(tensor, segment_id, segment_type):
    #     found_indexes = TransformerModule.index_search(tensor, segment_id)
    #     type_tensor = torch.full_like(found_indexes[:, 1:], segment_type, dtype=torch.int64)
    #     out_tensor = torch.cat([found_indexes, type_tensor], dim=1, out=None)
    #     return out_tensor  # return [[dim1_index, dim2_index, type_of_segment]]

    # def segments_merge(tensors):
    #     tensor = torch.cat(tensors, dim=0)
    #     # sorting by 1 colunm
    #     tensor = tensor.index_select(0, tensor[:, 1].sort()[1])
    #     # sorting by 2 colunm
    #     tensor = tensor.index_select(0, tensor[:, 0].sort()[1])
    #     return tensor  # return [[dim1_index, dim2_index, type_of_segment]]

    # def segment_embed_create(like_tensor, segments_tensor):
    #     seg_emb_tensor = torch.zeros_like(like_tensor, dtype=torch.long)
    #     index_tensor = segments_tensor[:, 1:2]

    #     index_tensor = torch.where(index_tensor != 0, index_tensor + 1, index_tensor)
    #     segments_tensor[:, 1:2] = index_tensor

    #     # location is [dim1_index, dim2_start_index, dim2_stop_index, type_of_segment]
    #     for location in segments_tensor:
    #         seg_emb_tensor[location[0], location[1]:location[2]+1] = location[3]
    #     return seg_emb_tensor

    # def _gen_type_embed_inds(self, emb_ids_tensor):
    #     is_cuda = emb_ids_tensor.is_cuda
    #     emb_ids_tensor = emb_ids_tensor.cpu()

    #     left_index_tensor = TransformerModule.index_search(emb_ids_tensor, self.sep_token_id, with_first_items=True)
    #     right_index_tensor = TransformerModule.index_search(emb_ids_tensor, self.sep_token_id, with_last_items=True)
    #     segment_tensors = []
    #     segment_tensors.append(TransformerModule.segment_search(emb_ids_tensor, self.info_bos_id, 0))
    #     segment_tensors.append(TransformerModule.segment_search(emb_ids_tensor, self.talker1_bos_id, 1))
    #     segment_tensors.append(TransformerModule.segment_search(emb_ids_tensor, self.talker2_bos_id, 2))
    #     segment_tensors.append(TransformerModule.segment_search(emb_ids_tensor, self.bos_token_id, 3))
    #     segments_tensor = TransformerModule.segments_merge(segment_tensors)
    #     # Concat to [[dim1_index, dim2_start_index, dim2_stop_index, type_of_segment]]

    #     segments_tensor = segments_tensor[:, -1:]
    #     if left_index_tensor.shape[0] != segments_tensor.shape[0]:
    #         repeated_tensor = segments_tensor[:1, :].repeat(left_index_tensor.shape[0], 1)
    #         repeated_tensor[:segments_tensor.shape[0], :segments_tensor.shape[1]] = segments_tensor
    #         segments_tensor = repeated_tensor
    #     segments_tensor = torch.cat([left_index_tensor, right_index_tensor[:, 1:], segments_tensor, ], dim=1)
    #     seg_emb_ids_tensor = TransformerModule.segment_embed_create(emb_ids_tensor, segments_tensor)
    #     if is_cuda:
    #         seg_emb_ids_tensor = seg_emb_ids_tensor.cuda()

    #     return seg_emb_ids_tensor
